...
from wah.wah_collector_llm import WahCollectorLLM
from alfred.alfred_collector_llm import AlfredCollectorLLM
...
def main(cfg):
    ...
    if cfg.dataset_type == 'alfred':
        ...
        if cfg.exp_type == 'collect_llm':
            collector = AlfredCollectorLLM(cfg)
        else:
            raise NotImplementedError()
    elif cfg.dataset_type == 'wah':
        cfg.dataset.collect_dir = os.path.join(cfg.dataset.collect_ex_root_dir, cfg.exp_type, f'{cfg.task_planner}{"_wm" if cfg.llm_agent.working_memory else ""}')
        if cfg.exp_type == 'collect_llm':
            collector = WahCollectorLLM(cfg)
        else:
            raise NotImplementedError()
    else:
        raise NotImplementedError()
